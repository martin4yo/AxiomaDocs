...
// Script de migración con DEBUG para identificar problemas
...
async function main() {
  ...
  try {
    console.log('🚀 Iniciando migración con DEBUG\n');
    ...
    // LIMPIAR TODO EN POSTGRESQL (con confirmación)
    ...
    // 2. MIGRAR ESTADOS (Sin foreign keys a usuarios por ahora)
    console.log('\n=== MIGRANDO ESTADOS ===');
    ...
    if (states.length > 0) {
      console.log('Estructura del primer estado:', Object.keys(states[0]));
    }
    ...
    // Ver estructura del primer documento
    if (docs.length > 0) {
      console.log('Campos disponibles:', Object.keys(docs[0]));
      console.log('Primer documento ejemplo:', docs[0]);
    }
    ...
    for (const doc of docs) {
      try {
        const data = {
          ...
          estadoId: doc.estadoId || doc.estadoVencimientoId || defaultStateId,
          activo: true,
          createdBy: 1,
          updatedBy: 1,
        ...
        console.log(`Creando documento ${doc.codigo} con estadoId ${data.estadoId}`);
    ...
    if (resources.length > 0) {
      console.log('Campos disponibles:', Object.keys(resources[0]));
    }
    ...
    if (entities.length > 0) {
      console.log('Campos disponibles:', Object.keys(entities[0]));
    }
